...
export async function GET(request: NextRequest) {
  ...
  const result = await tryCatch(
      ...
      const results = await Promise.allSettled(
        unpinnedMetadata.map(async (item) => {
          try {
            ...
            if (item.featuredImage) metadataJson.featured_image = item.featuredImage;
            ...
            if (item.backgroundColor) metadataJson.background_color = item.backgroundColor;
    ...
    {
      errorMessage: "Metadata IPFS pinning cron job failed",
      shouldLog: true,
    }
  );
